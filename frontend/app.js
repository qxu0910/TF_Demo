...
$('#clear-chat').addEventListener('click', () => { if (busy) return; $('#messages').innerHTML = welcomeMarkup; document.querySelectorAll('[data-prompt]').forEach(button => button.addEventListener('click', () => { $('#prompt').value = button.dataset.prompt; $('#chat-form').requestSubmit(); })); [0, 1, 2].forEach(step => trace(step, '')); $('#trace-status').textContent = '等待请求'; $('#request-id').textContent = 'REQ / —'; $('#request-id').title = ''; $('#elapsed').textContent = '—'; $('#queue-time').textContent = '—'; $('#compute-time').textContent = '—'; });
