...
主体层：L7 C++ 服务。上游 L8 Java，下游默认是 CPU 教学计算，可选 CUDA 字节归约。L4 的编译与内存练习保留在 runtime_demo 中。
...
尚无真实模型、流式输出、Token 配额、鉴权、动态批处理或分布式通信。C++ HTTP/JSON 依赖版本已固定；升级时需更新校验值并重跑测试。阶段耗时不可直接外推为模型吞吐。

CUDA 构建和设备验收见 [算子教程](../cuda-kernels/README.md)。启动自检失败时检查 nvcc、驱动与设备；CUDA 编译通过不等于 GPU 实测通过。
